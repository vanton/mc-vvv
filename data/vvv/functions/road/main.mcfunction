...
# 2. 在武器架左右紧贴的位置放置命令方块，每个方块上方放置按钮用于启动
# 3. 建造桥的方块内容为：function vvv:road/build
# 4. 移除桥的方块内容为：function vvv:road/demolish
# 5. 命令方块前需要一个空位用于启动时放置传递红石信号的方块
# 6. 在命令方块前放置脉冲红石信号源，建议最简化的两个背对背的红石探测器，可使用红石中继器控制延时
# 参考 1.png
################################################################################
#
